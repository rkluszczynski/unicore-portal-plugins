...
public class ResponseDocumentWrapper {
    public static final String TRUST_DELEGATION_ATTRIBUTE_NAME = "TrustDelegationOfUser";
    ...
    private ResponseAttributes attributes = new ResponseAttributes();

    public ResponseDocumentWrapper(ResponseDocument responseDocument) throws IOException, XmlException {
        AssertionDocument[] assertionDocuments = SAMLUtils.getAssertions(
        ...
        for (AssertionDocument document : assertionDocuments) {
            ...
            if (assertion.sizeOfAttributeStatementArray() > 0) {
                ResponseAttributes newAttributes = new ResponseAttributes();
